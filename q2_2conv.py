...
def model(X, w, w_2, w_fc, w_o, p_keep_conv, p_keep_hidden):
    ...
    l2a = tf.nn.relu(tf.nn.conv2d(l1, w_2,                       # l1a shape=(?, 28, 28, 32)
                        strides=[1, 1, 1, 1], padding='SAME'))
    l3 = tf.reshape(l2a, [-1, w_fc.get_shape().as_list()[0]])    # reshape to (?, 14x14x32)
...
w_2 = init_weights([3, 3, 3, 32])       # 3x3x3 conv, 32 outputs
...
tf.summary.histogram("model",py_x)
    ...
    for i in range(100):
        ...
        print("iteration: ", i, "Accuracy: ", np.mean(np.argmax(teY[test_indices], axis=1) ==
                         sess.run(predict_op, feed_dict={X: teX[test_indices],
                                                         p_keep_conv: 1.0,
                                                         p_keep_hidden: 1.0})))
        writer.add_summary(sess,global_step=i)
        saver.save(sess,"checkpoints/session.ckpt")